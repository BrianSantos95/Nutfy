...
export const PlanPreview: React.FC = () => {
  ...
  const handleExport = () => {
    // 1. Validação de Perfil e Logo
    if (!profile || !profile.name || !profile.logoUrl) {
       alert("Preencha corretamente os dados antes de gerar o PDF. (Configure seu nome e logo no Perfil)");
       navigate('/profile');
       return;
    }
    ...
    if (!student || !student.name || meals.length === 0) {
      alert("Preencha corretamente os dados antes de gerar o PDF. (Verifique se há refeições cadastradas)");
      return;
    }
